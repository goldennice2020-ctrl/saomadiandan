...
public class MemberAuthServiceImpl implements MemberAuthService {
    ...
    public AppAuthLoginRespVO mobileLogin(AppAuthMobileLoginReqVO reqVO) {
        String userIp = getClientIP();
        String from = StrUtil.isBlank(reqVO.getFrom()) ? LoginTypeEnum.ROUNTINE.getValue() : reqVO.getFrom();
        MemberUserDO user = null;

        if (StrUtil.isNotBlank(reqVO.getOpenid())) {
            LambdaQueryWrapper<MemberUserDO> lambdaQueryWrapper = new LambdaQueryWrapper<>();
            if (LoginTypeEnum.ROUNTINE.getValue().equals(from)) {
                lambdaQueryWrapper.eq(MemberUserDO::getRoutineOpenid, reqVO.getOpenid());
            } else {
                lambdaQueryWrapper.eq(MemberUserDO::getOpenid, reqVO.getOpenid());
            }
        ...
        if (user == null) {
            user = userService.createUserIfAbsent(reqVO.getMobile(), userIp, from);
            Assert.notNull(user, "获取用户失败，结果为空");
