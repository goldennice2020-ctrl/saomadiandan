	...
	<view class="pay-page container position-relative">
		<view class="pay-page__content">
			<view class="pay-page__section">
				<list-cell class="pay-page__cell pay-page__cell--location">
					<view class="flex-fill d-flex justify-content-between align-items-center">
						<view class="store-name flex-fill">{{ orderType == 'takeout' ? '点餐外带' : '点餐堂食' }}</view>
			...
			<view class="pay-page__section">
				...
				<template>
					<list-cell arrow class="pay-page__cell pay-page__cell--meal-time">
						<view class="flex-fill d-flex justify-content-between align-items-center"
					...
					<list-cell class="pay-page__cell pay-page__cell--contact" last :hover="false">
						<view class="flex-fill d-flex justify-content-between align-items-center">
...
const  takeoutTIme = ref(false) // 外带取餐时间picker
...
const takeinTIme = ref(false) // 堂食时间selector
...
const pay = async() => {
	...
	const savedTableNo = uni.getStorageSync('tableNo')
	const savedTableCode = uni.getStorageSync('tableCode')
	const tableRemark = savedTableNo ? `桌号：${savedTableNo}号桌${savedTableCode ? `（${savedTableCode}）` : ''}` : ''
	const remark = [tableRemark, form.value.remark].filter(Boolean).join(' ')
