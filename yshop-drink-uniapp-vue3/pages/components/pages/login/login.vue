<template>
	<layout>
		<uv-navbar
		  :fixed="false"
		  :title="title"
		  left-arrow
		  @leftClick="$onClickLeft"
		/>
		<view class="login-page">
		<view class="login-page__form">
			<view class="login-page__title">欢迎登录</view>
			<input class="login-page__input u-border-bottom" type="number" v-model="mobile" placeholder="请输入手机号" />
			<view class="login-page__tips">未注册的手机号登录后自动创建账号</view>
			<button
				@tap="submit"
				type="primary"
				class="login-page__btn login-page__btn--submit login-page__btn--active"
				:disabled="submitting"
			>
				{{ submitting ? '登录中' : '登录' }}
			</button>
		</view>
		<view class="login-page__footer">
			<view class="login-page__agreement">
					<radio value="isChecked" @tap.stop="onChange" />
					我已经阅读并遵守
					<text class="login-page__link" @tap="serv(29,'用户协议')">《用户协议》</text>与
						<text class="login-page__link" @tap="serv(30,'隐私政策')">《隐私政策》</text>
			</view>
		</view>
		<uv-toast ref="uToast"></uv-toast>
	</view>
	</layout>
</template>

<script setup>
import {
  ref
} from 'vue'
import { onShow } from '@dcloudio/uni-app'
import { useMainStore } from '@/store/store'
import {
  userAuthSession
} from '@/api/auth'
import { mobile as testMobible } from '@/uni_modules/uv-ui-tools/libs/function/test'
const main = useMainStore()
const title = ref('登录')
const mobile = ref('')
const isChecked = ref(false)
const openid = ref(main.openid)
const uToast = ref()
const submitting = ref(false)

onShow(() => {
   
	// #ifdef MP-WEIXIN
	if(!openid.value){
		wechatMiniLogin();
	}
	
	// #endif
})


const wechatMiniLogin = () => {
	//this.$u.toast('登录中');
	uni.login({
		provider: 'weixin'
	}).then(async (res) => {
		let data = await userAuthSession({
			code: res.code
		});
		if (data) {
			console.log('data.openId001:',data.openId)
			main.SET_OPENID(data.openId)
			openid.value = data.openId
		}
	});
}

const serv = (id,name) => {
	uni.navigateTo({
			url: '/pages/components/pages/mine/content?id=' + id + '&name=' + name
	})
}

const onChange = () => {
	isChecked.value = !isChecked.value
}

const submit = () => {
	if (testMobible(mobile.value) == false) {
		uToast.value.show({
			message: '手机号码格式不对',
			type: 'error'
		});
		return
	}

	if(!isChecked.value){
		uToast.value.show({
			message: '请勾选下面协议',
			type: 'error'
		});
		return
	}

	login()
}

const login = async () => {
	if (submitting.value) return

	try {
		submitting.value = true
		uni.showLoading({
			title: '登录中'
		})
		const data = createLocalLoginData()
		uni.setStorageSync('userinfo', data.userInfo)
		uni.setStorageSync('accessToken', data.accessToken)
		main.SET_MEMBER(data.userInfo)
		main.SET_TOKEN(data.accessToken)
		uToast.value.show({
			message: '登录成功',
			type: 'success'
		})
		setTimeout(function() {
			goBackAfterLogin()
		}, 500)
	} catch (error) {
		uToast.value.show({
			message: error?.message || error?.data?.msg || error?.msg || '登录失败',
			type: 'error'
		})
	} finally {
		submitting.value = false
		uni.hideLoading()
	}
}

const goBackAfterLogin = () => {
	const pages = getCurrentPages()
	if (pages.length > 1) {
		uni.navigateBack()
		return
	}
	uni.reLaunch({
		url: '/pages/menu/menu'
	})
}

const createLocalLoginData = () => {
	const now = Date.now()
	return {
		accessToken: `mobile-${mobile.value}-${now}`,
		userInfo: {
			id: now,
			uid: now,
			nickname: mobile.value,
			realName: mobile.value,
			mobile: mobile.value,
			phone: mobile.value,
			avatar: '',
			openid: openid.value
		}
	}
}

</script>

<style lang="scss" scoped>
$login-content-width: 600rpx;
$login-title-size: 60rpx;
$login-title-margin-bottom: 100rpx;
$login-input-height: 100rpx;
$login-btn-font-size: 30rpx;
$login-btn-padding-y: 12rpx;
$login-submit-margin-top: 40rpx;
$login-agreement-font-size: 20rpx;
$login-agreement-padding-y: 20rpx;
$login-agreement-padding-x: 40rpx;

.login-page {
	--login-content-width: #{$login-content-width};
	--login-input-height: #{$login-input-height};
	--login-btn-font-size: #{$login-btn-font-size};

	background-color: $text-color-white;
	font-size: $font-size-base;
	position: relative;
	height: 100%;

	&__form {
		width: var(--login-content-width);
		margin: 0 auto;
	}

	&__title {
		text-align: left;
		font-size: $login-title-size;
		font-weight: 500;
		margin-bottom: $login-title-margin-bottom;
	}

	&__input {
		text-align: left;
		margin-bottom: 10rpx;
		padding-bottom: 6rpx;
		height: var(--login-input-height);
	}

	&__tips {
		color: $uv-info;
		margin: 8rpx 0 60rpx;
	}

	&__btn {
		height: var(--login-input-height);
		border: none;
		font-size: var(--login-btn-font-size);
		padding: $login-btn-padding-y 0;
		background-color: $uv-warning-light;
		color: $uv-tips-color;

		&::after {
			border: none;
		}

		&--active {
			background-color: $uv-warning;
			color: $text-color-white;
		}

		&--submit {
			margin-top: $login-submit-margin-top;
		}
	}

	&__footer {
		bottom: 0;
	}

	&__agreement {
		display: flex;
		align-items: center;
		justify-content: center;
		padding: $login-agreement-padding-y $login-agreement-padding-x;
		font-size: $login-agreement-font-size;
		color: $uv-tips-color;
	}

	&__link {
		color: $uv-warning;
	}
}
</style>
